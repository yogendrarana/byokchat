    ...
    handlers: {
      POST: async ({ request }) => {
        try {
          const body: {
            threadId?: string;
            messages: Array<Partial<MessagesInsert>>;
            modelId: string;
            providerId: TCoreProvider;
          } = await request.json();
          ...
          if (!Array.isArray(messages) || messages.length === 0) {
            return json(
              { success: false, message: "Missing messages array" },
              { status: 400 }
            );
          }
          ...
          if (
            !Array.isArray(lateseMessageParts) ||
            lateseMessageParts.length === 0
          ) {
            return json(
              {
                success: false,
          ...
          if (!userActiveKeys.length) {
            return json(
              {
                success: false,
                message: `No active api key found for ${providerId}`,
              },
              { status: 401 }
            );
          }
          ...
          const apiKey =
            userActiveKeys[Math.floor(Math.random() * userActiveKeys.length)]
              .key;
          ...
          if (!finalModel) {
            return json(
              { success: false, message: "Invalid provider or model id" },
              { status: 401 }
            );
          }
                ...
                if (isImageModel) {
                  ...
                  await db.transaction(async (trx) => {
                    ...
                    await trx
                      ...
                      .where(
                        eq(messageSchema.id, created.newAssistantMessageId)
                      );
                  });
                  return;
                }
